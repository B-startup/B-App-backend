...
describe('ProjectTagService', () => {
  ...
  describe('createAssociation', () => {
    ...
    it('should create association successfully', async () => {
      (mockPrismaService.projectTag.findFirst as jest.Mock).mockResolvedValue(null);
      (mockPrismaService.project.findUnique as jest.Mock).mockResolvedValue(mockProject);
      (mockPrismaService.tag.findUnique as jest.Mock).mockResolvedValue(mockTag);
      (mockPrismaService.projectTag.create as jest.Mock).mockResolvedValue(mockProjectTag);
    ...
    it('should throw ConflictException when association already exists', async () => {
      (mockPrismaService.projectTag.findFirst as jest.Mock).mockResolvedValue(mockProjectTag);
    ...
    it('should throw NotFoundException when project not found', async () => {
      (mockPrismaService.projectTag.findFirst as jest.Mock).mockResolvedValue(null);
      (mockPrismaService.project.findUnique as jest.Mock).mockResolvedValue(null);
  ...
  describe('addMultipleTagsToProject', () => {
    it('should add multiple tags to project successfully', async () => {
      ...
      const result = await service.addMultipleTagsToProject('test-project-id', tagIds);
  ...
  describe('findByProject', () => {
    it('should return associations for a project', async () => {
      (mockPrismaService.projectTag.findMany as jest.Mock).mockResolvedValue([mockProjectTag]);
  ...
  describe('removeAssociation', () => {
    it('should remove association successfully', async () => {
      (mockPrismaService.projectTag.findFirst as jest.Mock).mockResolvedValue(mockProjectTag);
      (mockPrismaService.projectTag.delete as jest.Mock).mockResolvedValue(mockProjectTag);

      const result = await service.removeAssociation('test-project-id', 'test-tag-id');

      expect(mockPrismaService.projectTag.findFirst).toHaveBeenCalledWith({
        where: { projectId: 'test-project-id', tagId: 'test-tag-id' },
      });
  ...
  describe('countByProject', () => {
    it('should return count of associations for a project', async () => {
      (mockPrismaService.projectTag.count as jest.Mock).mockResolvedValue(5);
  ...
  describe('countByTag', () => {
    it('should return count of associations for a tag', async () => {
      (mockPrismaService.projectTag.count as jest.Mock).mockResolvedValue(3);
  ...
  describe('findPopularTags', () => {
    it('should return popular tags with project counts', async () => {
      ...
      (mockPrismaService.projectTag.groupBy as jest.Mock).mockResolvedValue(mockGroupBy);
    ...
    it('should return similar projects based on shared tags', async () => {
      ...
      const mockSimilarProjects = [
        { 
          ...
          tagId: 'tag1',
          project: { id: 'similar-project-1', title: 'Similar Project 1' }
        },
        { 
          projectId: 'similar-project-2', 
          tagId: 'tag2',
          project: { id: 'similar-project-2', title: 'Similar Project 2' }
        },
      ];
      ...
      const result = await service.findSimilarProjects('test-project-id', 5);
